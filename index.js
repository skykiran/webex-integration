...
*/
let gitCardAndValues = require('./cardTypeCheck');
let checkRepoPermission = require('./gitApiCall');;

framework.on('attachmentAction', async (bot, trigger) => {
  console.log('read Action', trigger.attachmentAction);
  await gitCardAndValues.cardTypeCheck(bot, trigger.attachmentAction);
});
...
framework.hears(
  "git", 
  (bot, trigger)=>{
    console.log("GitHub Details requested....");
    const repoCountCard = require("./acard");
    bot.sendCard(repoCountCard.gitRepoCountCard);  
...
framework.hears(
  ...
  (bot, trigger) => {
    ...
    const card = require("./acard"); //loading the card
    bot.sendCard(card.giphCard);
  },
  "**giph**: (get a GIPH for yourself) ",
  0
);
